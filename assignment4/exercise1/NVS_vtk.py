...
def save_to_vtk(filename, Mass, Momx, Momy, Energy, gamma, vol):
    """
    Save the evolving simulation data as a VTK file for visualization in ParaView.
    The function extracts the primitive variables before saving.
    """
    ...
    # Flatten data properly for VTK
    rho_flat = rho.T.flatten()
    vx_flat = vx.T.flatten()
    vy_flat = vy.T.flatten()
    P_flat = P.T.flatten()
...
def main():
    ...
    while t < tEnd:
        ...
        # Save VTK files at specific output intervals
        if plotThisTurn or (t >= tEnd):
            vtk_filename = f"frame_{outputCount:03d}.vtk"
            save_to_vtk(vtk_filename, Mass, Momx, Momy, Energy, gamma, vol)
            outputCount += 1
